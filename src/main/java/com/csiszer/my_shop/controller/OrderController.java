...
import com.csiszer.my_shop.service.order.IOrderService;
    ...
    public ResponseEntity<ApiResponse> getOrderById(@PathVariable Long orderId) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(new ApiResponse("Order not found by id", e.getMessage()));
        }
    }

    @GetMapping("/{useId}/order")
    public ResponseEntity<ApiResponse> getUserOrders(@PathVariable Long useId) {
        try {
            List<OrderDto> orders = orderService.getUsersOrders(useId);
